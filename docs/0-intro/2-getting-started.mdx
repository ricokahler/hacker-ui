...
```
npm install hacker-ui@next
```

or

```
yarn add hacker-ui@next
```
...
Then wrap your app in a `<ThemeProvider>` and a `<ColorProvider />` from `react-style-system`. Here we'll do it from the index file.
...
render(
  <ThemeProvider theme={theme}>
    <ColorContextProvider
      // If you don't specify a color for your component, it'll default to the
      // this color. The accent color is recommended
      color={theme.accent}
      // If don't specify a surface color, it'll default to the surface color
      // provided. A surface color of black would work well for dark modes
      surface="#fff"
    >
      <App />
    </ColorContextProvider>
